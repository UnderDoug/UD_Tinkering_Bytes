...
{
    [HarmonyPatch]
    public static class UD_VendorKnownRecipe_Patches
    {
        private static bool doDebug => getClassDoDebug(nameof(UD_VendorKnownRecipe_Patches));

        [HarmonyPatch(
            declaringType: typeof(UD_VendorKnownRecipe),
            methodName: nameof(UD_VendorKnownRecipe.HandleEvent),
        ...
        {
            bool doVomit = false;
            string patchMethodName = $"{nameof(UD_VendorKnownRecipe_Patches)}.{nameof(UD_VendorKnownRecipe.HandleEvent)}({nameof(GetShortDescriptionEvent)})";
