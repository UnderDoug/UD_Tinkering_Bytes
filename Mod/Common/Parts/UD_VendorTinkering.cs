...
namespace XRL.World.Parts
{
        ...
        , IModEventHandler<UD_EndTradeEvent>
    {
        ...
        public static bool IsStockDataDisk(GameObject GO)
            => GO.HasPart<DataDisk>() && GO.HasProperty("_stock");
                ...
                if (TinkerData.PartName == nameof(ModMasterwork) || TinkerData.PartName == nameof(ModLegendary))
                {
                    if (!Vendor.TryGetPart(out HasMakersMark hasMakersMark))
                    {
                        ...
                        // string learnedMasterworkMsg = 
                            ("=subject.Refname= =subject.verb:have= learned how to tinker " + TinkerData.DisplayName + " items! " +
            ...
            Predicate<TinkerData> Filter = null)
        {
            if (Vendor != null && Vendor.TryGetPart(out UD_VendorTinkering vendorTinkering))
            {
                ...
                if (!dataDiskObjects.IsNullOrEmpty())
                    foreach (GameObject dataDiskObject in dataDiskObjects)
                        if (dataDiskObject.TryGetPart(out DataDisk dataDisk)
                            && (IgnoreSkillRequirement
                                || Vendor.HasSkill(dataDisk.GetRequiredSkill()))
                            && (Filter == null
                                || Filter(dataDisk.Data)))
        ...
        public IEnumerable<TinkerData> GetKnownRecipes(Predicate<TinkerData> Filter = null, bool IncludeInstalled = true)
        {
            ...
            if (IncludeInstalled
                && !InstalledRecipes.IsNullOrEmpty())
            {
        ...
        public bool DraftDataDisk(TinkerData TinkerData, out GameObject DataDisk)
            => DraftDataDisk(ParentObject, TinkerData, out DataDisk);

        public bool DraftDataDisk(TinkerData TinkerData)
            => DraftDataDisk(ParentObject, TinkerData, out _);
        ...
        public static int GetTinkerTierFromBit(char Bit)
            => GetTinkerTierFromSkillName(GetTinkerSkillFromBit(Bit));

        public static int GetTinkerTierFromRecipe(TinkerData TinkerData)
            => GetTinkerTierFromSkillName(DataDisk.GetRequiredSkill(TinkerData.Tier));

        public static string GetTinkerSkillFromBit(char Bit)
            => DataDisk.GetRequiredSkill(BitType.GetBitTier(Bit));

        public static string GetTinkerSkillFromBitHumanReadable(char Bit)
            => DataDisk.GetRequiredSkillHumanReadable(BitType.GetBitTier(Bit));
        ...
        public static bool FillBitRaffle(
            ref Raffle<char> BitRaffle,
            int Weight,
            int? Tier = null,
            int? TierCap = null
            )
        ...
        public static BitLocker GiveRandomBits(
            GameObject Tinker,
        ...
        {
            if (Vendor == null
                || Vendor.GetIntProperty(nameof(LearnByteRecipes)) > 0
                || !Vendor.HasSkill(nameof(UD_Basics)))
                return false;
        ...
        {
            if (Vendor == null
                || !EnableGiantsAllKnowModGiganticIfTinkerableAtAll)
                return false;
        ...
        public static string GetVendorRecipeSeed(GameObject Vendor)
            => The.Game?.GetWorldSeed() + "-" + Vendor.ID;
            ...
            )
        {
            ...
            static bool canLearn(TinkerData TinkerData, string TinkeringSkill, bool IncludeBasic, List<TinkerData> KnownRecipes)
            {
                if (KnownRecipes == null
                    || KnownRecipes.Any(r => r.IsSameDatumAs(TinkerData)))
                    return false;
        ...
        public static bool IsRecipeLearnable(
            GameObject Vendor,
            TinkerData TinkerDatum,
            IEnumerable<TinkerData> KnownRecipes
            )
            => Vendor.HasSkill(DataDisk.GetRequiredSkill(TinkerDatum.Tier))
            ...
            bool Silent = false)
        {
                ...
                && !(InstalledRecipes != null && InstalledRecipes.Contains(Recipe)))
            {
                if (!Silent)
                {
                    ...
                    string skillFailMsg = ("=subject.Refname= =verb:don't= have the required skill: " + requiredSkillName + "!")
                        .StartReplace()
                        .AddObject(Vendor)
                        .ToString();
            ...
            bool Multiple = false)
        {
            ...
            List<string> supplyOptions = new()
            {
                ...
                (askPay + " =subject.refname= to supply " + vendorBitCostString + ".")
                    .StartReplace()
                    .AddObject(Vendor)
                    .ToString(),
            };
            ...
            if (!TinkerData.Ingredient.IsNullOrEmpty())
            {
                ...
                if (ingredientObject == null)
                {
                    ...
                    else
                    {
                        ...
                        string noIngredientMsg = ("=subject.Refname= =subject.verb:don't= have the required ingredient: " + ingredientName)
                            .StartReplace()
            ...
            bool Silent = false)
        {
            if (!BitSupplierBitLocker.HasBits(BitCost))
            {
                string missingRequiredBitsMsg =
                    ("=subject.Refname= =subject.verb:don't= have the required <" + BitCost + "> bits!\n\n" +
            ...
            out List<GameObject> ApplicableItems)
        {
            ...
            if (ApplicableItems.IsNullOrEmpty())
            {
                string noApplicableItemsMsg =
                    ("=subject.Refname= =subject.verb:don't= have any items " +
            ...
            out List<bool> IsVendorOwnedRecipe)
        {
            ...
            {
                string noModsKnownMsg = "=subject.Refname= =subject.verb:don't= know any item modification recipes."
                    .StartReplace()
                    .AddObject(Vendor)
                    .ToString();
            ...
            bool Silent = false)
        {
            if (!Shopper.CanAfford(DramsCost))
            {
                if (!Silent)
                {
                    ...
                    string baseFailMsg =
                        ("=shopper.Refname= =shopper.verb:don't= have the required " + dramsCostString +
                        " to have =vendor.refname= " + ToDoWhat);
            ...
            bool SetTinkerInvoiceHold = false)
        {
            ...
            List<string> messageList = new()
            {
                ...
                !DoWhat.IsNullOrEmpty() ? ("=vendor.Refname= will " + DoWhat + " for " + dramsCostString + " of fresh water.") : "",
                TinkerInvoice ?? "",
        ...
        public static bool VendorDoBuild(GameObject Vendor, TinkerInvoice TinkerInvoice, GameObject RecipeIngredientSupplier)
        {
            ...
            try
            {
                ...
                string itemTinkeredMsg = ("=subject.Refname= =subject.verb:tinker= up " + whatWasTinkeredUp + "!")
                    .StartReplace()
                    .AddObject(Vendor)
                ...
                if (TinkerInvoice.HoldForPlayer)
                {
                    ...
                    comeBackToPickItUp += 
                        ("\n\n" + "Once =subject.refname= =subject.verb:have= the drams for "
        ...
        public static bool VendorDoMod(GameObject Vendor, GameObject Item, TinkerInvoice TinkerInvoice, GameObject RecipeIngredientSupplier)
        {
            ...
            try
            {
                ...
                if (didMod)
                {
                    ...
                    string didModMsg = 
                        ("=subject.Refname= =subject.verb:mod= " + itemNameBeforeMod + " to be " +
        ...
        public static bool VendorDoIdentify(GameObject Vendor, GameObject UnknownItem, int DramsCost, string IdentifyWhat)
        {
            ...
            string identifiedMsg = ("=subject.Refname= =subject.verb:identify= " + IdentifyWhat + " as " + anIdentifiedItem + ".")
                .StartReplace()
                .AddObject(Vendor)
        ...
        public static bool VendorDoRecharge(GameObject Vendor, GameObject Item)
        {
            ...
            foreach (var rechargablePart in Item.GetPartsDescendedFrom<IRechargeable>())
            {
                ...
                {
                    string noBitsMsg =
                        ("=subject.Refname= =subject.verb:don't= have any " + bitTypeBitString.Pluralize() + ", " +
                ...
                string takesToRechargeString = ("It would take " + bitsToRechargeFullyString + " for =vendor.Refname= to fully recharge =subject.t=.")
                    .StartReplace()
                    .AddObject(Item)
                    .AddObject(player)
                        ...
                        TinkerInvoice: tinkerInvoice))
                {
                    ...
                    string rechargedMsg = ("=subject.Refname= " + partiallyOrFully + "=subject.verb:recharge= =object.t=" + endMark)
                        .StartReplace()
                        .AddObject(Vendor)
                        .AddObject(Item)
                        .ToString();
                ...
                && The.Player is GameObject player)
            {
                ...
                try
                {
                    if (E.Command == COMMAND_MOD || E.Command == COMMAND_BUILD)
                    {
                        ...
                        {
                            string hostileFailMsg = "=subject.Refname= can't tinker with hostiles nearby!"
                                .StartReplace()
                                .AddObject(vendor)
                                .ToString();
